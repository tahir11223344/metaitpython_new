...
import { getIndustries, deleteIndustry, mediaUrl } from "@/lib/Industry_api";
...
export default function IndustryListPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await getIndustries({ search, sortDir, page, size });
      setItems(data.items || []);
  ...
  function goEdit(id) {
    router.push(`/dashboard/industries/edit/${id}`);
  }
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      alert(e.message || "Failed to delete industry");
    }
  }
    ...
    <div className="mx-auto w-full max-w-7xl px-3 py-6 sm:px-4 sm:py-8">
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          ...
          <button
            onClick={() => router.push("/dashboard/industries/create")}
              ...
              {loading ? (
                ...
                items.map((it, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 text-gray-600">{it.slug}</td>
                    <td className="px-4 py-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-400">
                        <span>Created: {formatDate(it.created_at)}</span>
                        <span>Updated: {formatDate(it.updated_at)}</span>
                      </p>
