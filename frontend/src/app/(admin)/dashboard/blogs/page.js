...
import { getBlogs, deleteBlog, mediaUrl } from "@/lib/Blog_api";
...
export default function BlogListPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await getBlogs({ search, sortDir, page, size });
      setItems(data.items || []);
    ...
    <div className="mx-auto w-full max-w-7xl px-3 py-6 sm:px-4 sm:py-8">
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          ...
          >
            <span aria-hidden>＋</span> Add Blog
          </button>
        </div>
              ...
              {loading ? (
                ...
                items.map((b, idx) => (
                  ...
                  >
                    ...
                    <td className="max-w-[260px] px-4 py-4 font-medium text-gray-800">
                      {b.title}
                    </td>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <dl className="mt-2 space-y-1 text-xs text-gray-600">
                        <div className="flex flex-wrap gap-x-3 gap-y-1">
                          <span>Category: {b.category?.name || "-"}</span>
                          <span>Type: {b.type || "-"}</span>
                          {b.read_time ? <span>{b.read_time}</span> : null}
                        </div>
